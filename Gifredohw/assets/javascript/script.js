 $(document).ready(function() {
    // your code here
// create a variable to identify emotions
var AllEmotions= ['happy', 'depressed', 'angry', 'surprise', 'love', 'insane', 'amused', 'shame', 'terrified'];
...
function renderButtons(){
    ...
    for (var i = 0; i < AllEmotions.length; i++){ //for the length of AllEmotions array
        var kdcreatebutton = $('<button>') //creates the button
        kdcreatebutton.addClass('typeEmotion'); //adds class typeEmotion
        kdcreatebutton.attr('data-name', AllEmotions[i]);
        kdcreatebutton.text(AllEmotions[i]); //adds the name of the emotion to the button
        $('#buttonsView').append(kdcreatebutton); //adding it to the div to show on the page
    }
